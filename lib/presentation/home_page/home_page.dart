...
//
//           if (contactSnapshot.exists) {
//             List<dynamic> contacts = contactSnapshot['contacts'];
//
...
class ContactManager {
    ...
    try {
      ...
      if (position != null) {
        ...
        if (userId.isNotEmpty) {
          ...
          await contactCollection.doc(userId).collection('contacts').get();

          if (contactSnapshot.docs.isNotEmpty) {
            contactSnapshot.docs.forEach((contactDoc) async {
              String? deviceToken = contactDoc.get('deviceToken');
              if (deviceToken != null) {
                await sendNotification(deviceToken, position.latitude, position.longitude);
              }
            });
          }
        }
  ...
  Future<void> sendNotification(
      String deviceToken, double latitude, double longitude) async {
    try {
      ...
      print('Notification sent to device with token: $deviceToken');
    } catch (e) {
